...
export default function ShopCategory() {
    ...
    return (
        ...
        >
            ...
            <Link href='/shop' className="border-t flex items-center gap-4 w-full px-4 py-3 hover:bg-blue-100 dark:hover:bg-gray-600  text-gray-800 dark:text-gray-200"><LayoutGrid /> All Categories</Link>
            <motion.div
                ...
                {/* <ul>
                    {categories?.map((category, index) => (
                        category.subCategories ? (
                            <li key={category.id || index} className="border-b last:border-b-0 dark:border-gray-600">
                                <button
                                    onClick={() => toggleCategory(index)}
                                    className="flex items-center justify-between w-full px-4 py-3 hover:bg-blue-100 dark:hover:bg-gray-600  text-gray-800 dark:text-gray-200"
                                >
                                        ...
                                        >
                                            <ul className="pl-10 py-3 space-y-2">
                                                {category?.subCategories?.map((sub) => (
                                                    <li key={sub.id || sub.name} className="flex border-b last:border-b-0 dark:border-gray-600 py-2 items-center gap-5 text-gray-600 dark:text-gray-300">
                                                        {sub?.icon}
                            ...
                            <Link
                                ...
                                className="flex items-center justify-between w-full px-4 py-3 hover:bg-blue-100 dark:hover:bg-gray-600 rounded text-gray-800 dark:text-gray-200 border-b dark:border-gray-600 cursor-pointer"
                            >
                                <span className="flex gap-2">
                                    {category.icon}
                                    <span className="ml-2">{category.name}</span>
